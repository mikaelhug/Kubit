 ...
 */
export function PxeGate({ what, onReady, onClose }: { what: string; onReady: () => void; onClose: () => void }) {
  ...
  return (
    <Dialog title="Start the PXE server first" onClose={onClose} footer={<><button class="btn" onClick={onClose}>Cancel</button><button class="btn" onClick={check}>Check again</button></>}>
      ...
      <Code text={st?.command ?? 'sudo kubit pxe --iface en0'} />
...
export function usePxeGated<T>(run: () => Promise<T>, onDone: (r: T) => void, onError: (msg: string) => void) {
  const [gate, setGate] = useState<{ what: string } | null>(null)
  const attempt = (what: string) => run().then(onDone).catch((e) => { if (e?.code === 'pxe-down') setGate({ what }); else onError(e.message) })
  const element = gate ? <PxeGate what={gate.what} onClose={() => setGate(null)} onReady={() => { const w = gate.what; setGate(null); attempt(w) }} /> : null
