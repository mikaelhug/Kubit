...
export function Inventory() {
  ...
  const columns: Column<NodeRow>[] = [
    ...
    { id: 'model', header: 'Model', sort: (n) => `${n.inventory?.manufacturer ?? ''} ${n.inventory?.product ?? ''}`, cell: (n) => <span class="text-muted truncate max-w-[180px] block">{[n.inventory?.manufacturer, n.inventory?.product].filter(Boolean).join(' ') || '—'}</span> },
  ...
  return (
    <div class="p-6 flex flex-col gap-5">
      <Section title="Inventory" help="Every machine Kubit has seen, identified by its uplink MAC (plus SMBIOS UUID and serial when reported). Machines booted from a Talos ISO or over PXE wait in maintenance mode on port 50000; scanning records their hardware so they can be adopted. A machine that moves to a new DHCP address keeps its row."
        actions={<a class="btn" href="https://factory.talos.dev" target="_blank" rel="noreferrer" title="Image Factory: download a Talos ISO for the USB path">Talos ISO ↗</a>}>
        ...
        <DataTable id="inventory" columns={columns} rows={nodes} rowKey={(n) => n.mac || n.ip} defaultSort={{ id: 'ip', dir: 'asc' }} empty="No machines known yet. Scan a subnet." />
      </Section>
